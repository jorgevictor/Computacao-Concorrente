...
#define O 3 //numero de threads escritoras e leitoras
...
void * escritorLeitor (void * arg) {
  int *id = (int *) arg;
  while(1) {
    InicLeit(*id);
    printf("Escritora-Leitora %d esta lendo\n", *id);
    FimLeit(*id);
    sleep(1);
    InicEscr(*id);
    printf("Escritora-Leitora %d esta escrevendo\n", *id);
    FimEscr(*id);
...
int main(void) {
  ...
  //cria as threads escritoras e leitoras
  for(int i=0; i<O; i++) {
    id[i+L+E] = i+1;
    if(pthread_create(&tid[i+L], NULL, escritorLeitor, (void *) &id[i+L])) exit(-1);
  } 
